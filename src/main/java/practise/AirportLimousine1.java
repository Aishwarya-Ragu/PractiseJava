...
public class AirportLimousine1 {
    static int onwardCount =0;
    ...
    public static int collectMax(List<List<Integer>> mat) {
        ...
        int i =0;
        int j =0;
        onwardCount =  recurseList(mat,i,j,row,col,0, "onward");
        returnCount =  recurseList(mat,row-1,col-1,row,col,0,"return");
    ...
    public static int recurseList(List<List<Integer>> mat,int i, int j,int row,int col,int riderCount,String flag){
        ...
        if(flag=="onward"){
             ...
             onwardCount= Math.max(rightCount,downCount);
        }
        if(flag=="return"){
            ...
            returnCount = Math.max(leftRiderCount,upRiderCount);
        }
        return onwardCount + returnCount;
    }
